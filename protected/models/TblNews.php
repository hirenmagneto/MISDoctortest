...
class TblNews extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('news_title, news_title_slug, news_content, news_date, created_on, isactive', 'required'),
			array('isactive', 'numerical', 'integerOnly'=>true),
			array('news_title, news_title_slug', 'length', 'max'=>500),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('news_id, news_title, news_title_slug, news_content, news_date, created_on, isactive', 'safe', 'on'=>'search'),
		);
	}
