...
class TestQuestionController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['TestQuestion']))
		{
			...
			if($model->save())
				...
				if($insert_id != ''){
					$vl=0;
					foreach($_POST['txt_option'] as $key=>$value){
						
							$model1=new QuestionAnswer;
							$model1->question_id=$insert_id;
							$model1->question_option=$value;
							if(isset($_POST['chk_right_option_'.$key]))
							$model1->right_answer=$_POST['chk_right_option_'.$key];
							$model1->save();
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['TestQuestion']))
		{
			...
			if($model->save())
				//$this->redirect(array('view','id'=>$model->question_id));
				if($id != ''){
					QuestionAnswer::model()->deleteAll("question_id ='" .$id . "'");
					foreach($_POST['txt_option'] as $key=>$value){
						
							$model1=new QuestionAnswer;
							$model1->question_id=$id;
							$model1->question_option=$value;
							if(isset($_POST['chk_right_option_'.$key]))
							$model1->right_answer=$_POST['chk_right_option_'.$key];
							$model1->save();
	...
	public function actionDelete($id)
	{
		...
		QuestionAnswer::model()->deleteAll("question_id ='" .$id . "'"); // delete record from this table QuestionAnswer
